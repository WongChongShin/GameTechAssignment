...
public class enemyMovementExample : MonoBehaviour
{
    public float speed;
    GameObject[] grid;
    GameObject[] obstacle;
    ...
    {
        checkMoveVisited();
        changePossibleMoveObjectColour();
        calculateDistance();
    }
    ...
    void OnTriggerEnter(Collider collide)
    {
        ...
        for (int i = 0; i < grid.Length; i++)
        {
            if (collide.gameObject.name == grid[i].gameObject.name)
            {
                ...
                collide.gameObject.GetComponent<Renderer>().material.color = Color.black;
    ...
    void changePossibleMoveObjectColour()
    {
        for(int i = 0; i < possibleMoveObject.Count; i++)
        {
            GameObject.Find(possibleMoveObject[i]).GetComponent<Renderer>().material.color = Color.yellow;
        }
    }
